...
export function Navigation() {
  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled || isOpen ? 'bg-header-80 backdrop-blur-sm backdrop-saturate-50 border-panel-10 shadow-lg shadow-black/5' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto site-padding">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navLinks.map(link => <Link key={link.name} to={link.path} className={`text-sm font-medium transition-colors hover:text-cyan-400 ${location.pathname === link.path ? 'text-cyan-400' : 'text-secondary'}`}>
                {link.name}
              </Link>)}
          </nav>
      ...
      <AnimatePresence>
        ...
        height: 0
            ...
            <div className="px-4 pt-2 pb-6 space-y-2">
              {navLinks.map(link => <Link key={link.name} to={link.path} className={`block px-3 py-3 rounded-lg text-base font-medium ${location.pathname === link.path ? 'bg-panel-10 text-cyan-400' : 'text-secondary hover:bg-panel-5 hover:text-primary'}`}>
                  {link.name}
                </Link>)}
              <div className="pt-4 grid grid-cols-2 gap-4">
                ...
                <Link to="/recruiters" className="w-full">
